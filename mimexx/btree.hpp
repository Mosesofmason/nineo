...
//       bool operator= ( const TreeNode<TType>& data );
    ...
    private:
      ...
      std::map<TKey, TreeNode<TKey, TData>*> m_subjects;
      TreeNode<TKey, TData>* m_root;
    public:
      Tree ( const TKey& key, const TData& data );
      virtual ~Tree ();
      void add_subject ( const TKey& subject,TreeNode<TKey,TData>* node);
      TreeNode<TKey, TData>* find_subject ( const TKey& subject);
...
//     }
    ...
    template<typename TKey, typename TData>
    void Tree<TKey,TData>::add_subject( const TKey& subject,
					TreeNode<TKey,TData>* node)
    {
      m_subjects[subject] = node;
    }
    template<typename TKey, typename TData>
    TreeNode<TKey, TData>* Tree<TKey,TData>::find_subject( const TKey& subject)
    {
      std::map<TKey,TreeNode<TKey,TData>*>::iterator iter;
      iter = m_subjects.find (subject);
      if ( iter != m_subjects.end() ) return iter->second;
      return NULL;
    }
    template<typename TKey, typename TData>
